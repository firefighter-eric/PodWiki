...
PodWiki 是播客 Wiki，不是公开视频、访谈或长内容的通用归档。节目和单集必须分别
通过下面的边界检查；仅仅发布在 Bilibili、时长较长、多人对谈、标题含 `EP`，或
适合只听声音，都不能单独证明它是播客。

节目至少需要一项可核实的发布者证据：发布者页面明确称其为“播客”或 `Podcast`；
存在公开 RSS、Apple Podcasts、小宇宙等播客节目页；或 Bilibili 发布者建立了明确
命名为播客、且标示为正片的官方合集。频道、账号和机构本身不是节目身份；混合发布
账号只能把满足上述条件的播客节目作为白名单，不能把账号全部投稿视为播客。

单集必须是该已核实播客的一份完整节目，并能在发布者播客 feed、节目页或官方播客
正片合集中精确对应。音频播客的视频版可以收录；正式分支、圆桌、bonus 或现场版也
可以收录，但必须由发布者作为同一播客的完整单集发布。来源列表顺序、合集位置和
抓取顺序仍不能作为正式期号证据。

以下内容默认不收录；只有发布者也把同一完整内容作为该播客正式单集发布时才可例外：

- 精选片段、切片、预告、花絮、Shorts 和重复剪辑；
- 直播、直播回放、临时连麦、线下活动和大会录制；
- 演讲、主题分享、课程、教程、论文讲解、发布会和产品演示；
- 新闻、资讯、普通评论、单人口播、Vlog，以及账号内其他非播客视频。

对混合型频道做有界发现时，先按项目扫描剧集 skill 把来源覆盖、每项判定和证据保存到
`.cache/scans/<scan-id>/scan.json`；无法完整访问必需来源时保留 `partial` 或
`blocked`，不得报告“没有更新”。无法可靠判断的条目使用 `needs-review` 停在发现
阶段，不下载、不建档，也不因为“看起来像访谈”而纳入。必需发现面在覆盖窗口内观察到
的每个精确 URL 都要写入来源记录，并与四类判定一一对应，不能通过漏记条目制造完整扫描。

用户确认添加精确候选后，添加剧集 skill 才把批准子集冻结到
`.cache/intake/<show-id>/manifest.json`：Bilibili 记录频道规范 URL 与 `mid`、官方
播客 `season_id` 和合集标题、规范 BVID URL、来源 scan 路径与 SHA-256、冻结时间与
白名单总数；使用外部公开播客 feed 交叉核实时，还要记录 feed 规范 URL 及每个 BVID
对应的 GUID 和单集 URL。添加期间不得重新发现或扩展清单。
...
- 单集达到 Web 收录状态后才同时加入根索引与对应节目索引；处理中记录不得提前进入
  任一索引。已经进入索引的单集发生更新时，两个索引必须同步刷新。
- 总结列直接链接本地总结文件。selected 为中文单稿时，逐字稿列链接该 selected 文件；
  selected 为英文时，根索引和节目索引的逐字稿单元格都必须同时链接 selected 英文稿
  与 `transcript.zh-CN.md` 中文译稿，英文稿仍是 selected 原文。
...
profile 中的事实可以来自本集 `sources` 已登记的来源，也可以从本集 selected 完整
逐字稿中上下文明确的嘉宾自述整理。允许把分散在多处的成长经历、教育路径、任职
经历和研究方向归纳为 `headline`、`bio` 及结构化条目；使用尚未人工核对的机器逐字稿
时，`bio` 应以“据本期自述”等方式说明信息性质，不能写成已经独立核实的履历。

使用额外的发布者人物介绍、机构主页等材料时，先把其规范 URL 登记到 `sources`。
逐字稿中的学校、机构、职位和时间线必须表达清楚且相互一致；模糊音译、模型补全、
未说出的学位或专业、仅靠常识补出的公司关系不得写入。`status: current` 只能用于
录制时明确仍在持续的关系，并以 `checked_at` 表示本次来源或逐字稿核对日期。
...
时长、期号和内部 `episode_key`，完整标题通常不超过 40 个字符；当已核实的完整
人物姓名较长时，允许保留全部姓名和分隔符，题目最多 12 个字符，长度上限为
`max(40, 人物部分字符数 + 3 + 12)`，不得为了缩短标题删去嘉宾或改写姓名。发布者未改写原标题
...
`release_type` 允许值为 `regular`、`special`、`bonus`；预告不属于完整播客单集，
不得建档或进入 Web 收录集合。
...
`transcript.path` 必须继续指向 `transcript.en.md`；根索引和节目索引的逐字稿单元格
必须同时链接英文 selected 稿和中文译稿。译稿的 provenance 与审核状态只登记在
...
访问与传输上下文，不构成收录、处理或批量授权。cookie、token、浏览器配置和其他
凭据只能临时保存在被 Git 忽略的 `.cache/`，不得进入日志、sidecar、Markdown 或 Git。
...
检查；小宇宙每集仍须独立通过 `NORMAL`、`FREE`、非私密、`PUBLIC`、栏目身份、
enclosure/媒体 URL、公开 M4A 字节数与时长校验。任何不符合条件的单集必须拒绝并
单独报告。批次例外不允许跨节目，或枚举和获取会员专属、付费、私密、受地区限制及
非播客内容。
