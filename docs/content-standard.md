...
Bilibili 每集仍须独立通过匿名来源身份、访问状态、单 P 和字幕检查；小宇宙每集仍须
独立通过 `NORMAL`、`FREE`、非私密、`PUBLIC`、栏目身份、enclosure/媒体 URL、公开
M4A 字节数与时长校验。任何不符合条件的单集必须拒绝并单独报告。批次例外不允许
跨节目、使用登录态，或枚举和获取付费、私密及非播客内容。
