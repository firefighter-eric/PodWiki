...
对混合型频道做有界发现时，必须先在 `.cache/intake/<show-id>/manifest.json` 冻结
本次白名单及证据：Bilibili 频道规范 URL 与 `mid`、官方播客 `season_id` 和合集标题、
规范 BVID URL、冻结时间与白名单总数；使用外部公开播客 feed 交叉核实时，还要记录
feed 规范 URL 及每个 BVID 对应的 GUID 和单集 URL。无法可靠判断的条目停在发现阶段，
不下载、不建档，也不因为“看起来像访谈”而纳入。
...
频道、播放列表或播客栏目；只有用户明确授权一个已核实的单一播客后，才允许执行
有界批量导入。Bilibili 混合账号必须先按本标准第 0 节锁定官方播客正片合集，或用
公开播客 feed 逐集交叉对应，再冻结包含 `mid`、`season_id`、规范 BVID 和筛选证据的
manifest；不得把频道其他投稿扩进清单。小宇宙栏目 manifest 固定包含栏目 PID、每集
规范 URL 与 `eid`。之后逐集、限速处理，不在运行中自动扩展范围。
