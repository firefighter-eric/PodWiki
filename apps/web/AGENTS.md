...
- `navigationTitle` 的内容契约是“证据化人物 · 精简题目”：优先已核实嘉宾，
  否则使用明确的一般参与者，再否则使用本期实际出场主播。可以为视觉样式拆分人物
  与题目，但必须保持原始字符串顺序和完整的可访问文本，不得把参与者或主播改成嘉宾。
...
- 紧凑导航和发现列表的辅助时间只展示 `publishedDate`，不要附加节目时长；
  逐字稿搜索结果的定位时间戳除外。
- 首页最近更新目录使用三段式层级：左侧红色 `catalogKeyword`，正文黑色导航人物，
  下一行灰色精简题目。关键词必须来自内容元数据，不得在组件中猜测。
- 单个播客的节目页使用更丰富的三层编辑列表：首行红色导航人物和小字“播客名 · 日期”，
...
- 全局侧栏是播客来源的主选择器：来源链接使用 `/shows/<showId>`，选中来源后
  侧栏单集列表只展示该来源；正文目录不再重复一套来源筛选控件。来源范围与当前
  单集所属播客是两个独立状态：从“全部节目”进入单集后继续显示全部单集，从某档
  播客节目页进入单集后继续只显示该档；阅读单集、切换总结/逐字稿或点另一单集都
  不得隐式改变来源范围。直接打开或刷新单集深链时，默认使用该单集所属播客作为范围。
...
- 侧栏折叠、阅读字号和行宽继续使用浏览器本地偏好，但首次绘制前必须由根布局的同步脚本
  应用到 `<html>`；客户端状态与这些属性保持同步。不得为了读取偏好把静态根布局改成请求期
  动态渲染，也不得等 hydration 后才改变列宽、字号或行宽而造成可见闪动与整页重排。
...
- 所有单集总结的“核心观点”开头维护一张逻辑速览表。列名和行内容由该期议题
  ...
  的完整观点、依据、边界和原文定位保留。
- `workflow`、模型、路径、哈希和审核待办属于仓库内部状态。读者正文和全文搜索
  统一从总结的“一句话总结”开始，到“编辑记录”之前结束；“阅读边界”只写读者
  有用的证据、利益关系、时效性和风险限制。“编辑记录”完全隐藏，不得展示
  `draft`、`machine`、lineage、SHA、待回听或待校对等维护信息。七个二级标题的
  顺序由 Web 与 Python 门禁共同校验，不保留旧版混合边界标题的兼容路径。
